...
class _uploadScreen1State extends State<uploadScreen1> {
  ...
  void initiatePayment(amt) async {
                    ...
                    onPressed: () async {
                      if (_file != null) {
                        ...
                        try {
                          ...
                          String start=preference.getString("start") ?? "";
                          String end=preference.getString("end") ?? "";
                          String noofcopy=preference.getString("noofcopy") ?? "";
                          String color=preference.getString("color") ?? "";
                          String amount=preference.getString("amount") ?? "";

                          print("shareduser:$userId");
                          print("shareduser:$start");
                          print("shareduser:$end");
                          print("shareduser:$noofcopy");
                          ...
                          var response = await ApiService().uploadFile(
                            ...
                            _file!,
                            false,
                            false// Pass the file path
                          );
                          print('Upload response: $response');
                          initiatePayment(amount);
                          final response1 = await transactionApi().pay(userId, "print", amount);
